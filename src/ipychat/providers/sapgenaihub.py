...
class SAPGenAIHubProvider(BaseProvider):
    def initialize_client(self) -> None:
        ...
        llm = LLM(name=self.config["current"]["model"], version="latest", parameters={"max_tokens": 256, "temperature": 0.2})
        ...
        orchestration_service = OrchestrationService(api_url=api_key, config=config)

        self.client = orchestration_service
